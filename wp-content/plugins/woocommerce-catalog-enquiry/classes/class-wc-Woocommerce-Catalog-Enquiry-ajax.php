...
class WC_Woocommerce_Catalog_Enquiry_Ajax {
	...
	public function send_product_enqury_mail() {
		...
		if($product){
			...
			if($send_email->trigger( $email_admin, $enquiry_data ) || WP_DEBUG === true) {
				if(isset($_SESSION['variation_list'])){
					unset($_SESSION['variation_list']);
				}
				...
				global $wpdb;
				   ...
				   $wpdb->insert( 'woo_enquiry_master', array(
						'woo_user_name' => $enquiry_data['cust_name'], 
						'product_id' => $enquiry_data['product_id'], 
						'woo_user_email' => $enquiry_data['cust_email'], 
						'woo_user_subject' => $enquiry_data['subject'], 
						'woo_user_phone' => $enquiry_data['phone'], 
						'woo_user_address' => $enquiry_data['address'], 
						'woo_user_comment' => $enquiry_data['comment'], 
